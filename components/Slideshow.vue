<template>
  ...
  pour cacher un bouton mettre une classe non valide 
  ...
  -->
	<div class="mainSlider">
		...
		<!-- @touchend="userNav(-1)" -->
		<a class="slideBtn btnLeft" @click="userNav(-1)">
			<i :class="leftnav_class"></i>
		</a>
		<!-- @touchend="userNav(1)" -->
		<a class="slideBtn btnRight" @click="userNav(1)">
...
export default {
	...
	data() {
		return {
			...
			$el: null, //stock le doc.getElementById obtenu grâce à l'id ci dessus
			imgContainer: null,
	...
	computed: {
		dynamicwidth: {
			get() {
				return (
					"width :" +
					this.nbrOfslides * parseInt(this.slidewidth) +
					"% "
				);
			},
		},
		indexToreturn: {
			get() {
				return this.slide_ind;
			},
			set(val) {
				return this.$emit("update:slide_ind", val);
			},
		},
		toggleSlideshow: {
			get() {
				return this.inside_close;
			},
			set(val) {
				return this.$emit("update:inside_close", val);
			},
		},
	},
	...
	mounted() {
		...
		this.slidewidthInPx = Math.round(
			this.$el.clientWidth * (+this.slidewidth / 100)
		);
		this.imgContainer =
			this.$el.getElementsByClassName("imageContainer")[0];
		...
		this.mainImage = Math.floor((marLeft + 50) / slidewidth_Int);
		//création de nouveauslidewidth_Int éléments pour pallier le manque d'image sur une taille d'affichage trop grande
		let futurSize = this.nbrOfslides * slidewidth_Int;
		...
		while (futurSize <= 100 + 2 * marLeft) {
			...
			this.nbrOfslides = this.imgContainer.children.length;
			futurSize = this.nbrOfslides * slidewidth_Int;
		}
		...
		// atterir a l'image cliqué
		if (this.slide_ind && this.indexToreturn > 1) {
			for (let i = 0; i < this.indexToreturn - 1; i++) {
				this.nav(1);
			}
		}
	...
	methods: {
		...
		setClassName() {
			for (var j = 0; j < this.imgContainer.childNodes.length; j++) {
				if (this.imgContainer.children[j]) {
					for (
						var k = 0;
						k < this.imgContainer.children.length;
						k++
					) {
						if (
					...
					if (j === this.mainImage) {
						...
						if (
							...
							// this.imgContainer.children[j].classList.remove(this.activeclass);
						}
					}
					// }
					...
					if (j > this.mainImage) {
						...
						if (this.sliderheight === "very_auto") {
							this.setAutoHeight(this.imgContainer.children[j]);
						}
					}
				}
			}
		...
		setAutoHeight(slide) {
			...
			try {
				const elImg = slide.getElementsByTagName("img")[0];
				if (!elImg) {
					console.error("no image in this slide");
					throw "no image in this slide";
				}
				if (elImg.naturalHeight && elImg.naturalHeight != 0) {
					imgH = elImg.naturalHeight;
				} else {
					setTimeout(() => {
						this.setAutoHeight();
					}, 100);
			...
			this.$el.style.setProperty("--current_height", imgH + "px");
		},
		...
		goToSlide(imageIndex) {
			...
			while (
				this.imgContainer.children[3].classList.contains("prems") ==
				false
			) {
				this.nav(1);
			}
		...
		setHeight() {
			if (this.sliderheight === "very_auto") {
				...
				try {
					var imageEl =
						this.imgContainer.children[0].getElementsByTagName(
							"img"
						)[0];
					...
					imgH =
						imgH > window.innerHeight ? window.innerHeight : imgH;
					this.$el.style.setProperty("--current_height", imgH + "px");
					this.$el.style.height = "var(--current_height)";
				...
				this.$el.style.height = this.sliderheight;
		...
		findNextValue(direction) {
			...
			this.indexToreturn = nextValue;
		},
		animateSlide(direction) {
			if (direction == 1) {
				const xx = this.manualScrollX + this.slidewidthInPx;
				// console.log(this.slidewidthInPx);
				// console.log(this.manualScrollX);
					...
					if (
						...
						if (xORy === "x") {
							if (this.startXposition - currentX > 50) {
								this.direction = "E";
								this.imgContainer
									.getElementsByClassName("current_slide")[0]
									?.classList.add("going_east");
								this.imgContainer
									.getElementsByClassName("current_slide")[0]
									?.classList.remove("going_west");
							} else if (this.startXposition - currentX < -50) {
								this.direction = "O";
								this.imgContainer
									.getElementsByClassName("current_slide")[0]
									?.classList.add("going_west");
								this.imgContainer
									.getElementsByClassName("current_slide")[0]
									?.classList.remove("going_east");
							}
						} else {
