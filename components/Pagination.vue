		...
		<div class="navsystem Head" v-if="nbrOfPages > 1 && show_top_nav">
			<div class="caseBox">
				...
				<div
					...
					"
					title="Première page"
					:class="currentPage < multipleBase || showMultiple ? 'pag_disable' : ''"
				>
				...
				<template v-if="!showMultiple">
					<div
						...
						:class="index + firstVisiblePage - 1 === currentPage ? 'current_pag' : ''"
					>
				...
				<div @click="showMultiple = !showMultiple" v-if="nbrOfPages > multipleBase">
					<i>...</i>
				</div>
				<!-- last page -->
				<div
				...
				<div
					@click="navToPage(currentPage + 1)"
					title="Page suivante"
					:class="currentPage + 1 === nbrOfPages || showMultiple ? 'pag_disable' : ''"
				>
			...
			<div class="viewStyle" v-if="other_style">
				<i class="fas fa-th" @click="changeListingStyle(false)" :class="optionStyle == false ? 'activeStyle' : ''"></i>
				<i class="fas fa-list" @click="changeListingStyle(true)" :class="optionStyle == true ? 'activeStyle' : ''"></i>
			</div>
			<!-- show quantity -->
			<p v-if="show_qtt">
				<span>{{ seenItems() }} /</span>
...
export default {
	...
	props: {
		...
		other_style:
			String /*classe option pour changer le style des listes la classe s'ajoute au parent direct des elements dans le slot  */,
		show_qtt: Boolean,
	...
	computed: {
		...
		watch_current_page: {
			get() {
				return this.give_current_page;
			},
			set(val) {
				return this.$emit("update:give_current_page", val);
			},
		},
	},
	...
	mounted() {
		this.$nextTick(() => {
			...
			this.itemContainerEl = this.baseCompEl.getElementsByClassName("slot_container")[0];

			this.itemsQuantity = this.totalEls;
			this.nbrOfPages = Math.ceil(this.itemsQuantity / this.itemsByPage);

			this.showWhatToShow();

			// console.log(this.listingForPage);
		});
	},
	methods: {
		isLastMultiple() {
		...
		setfirstVisiblePageForLastItem() {
			if (this.nbrOfPages % (this.nbrOfPages / this.multipleBase) === 0) {
				this.firstVisiblePage = (this.nbrOfPages / this.multipleBase - 1) * this.multipleBase;
			} else {
				this.firstVisiblePage = Math.floor(this.nbrOfPages / this.multipleBase) * this.multipleBase - 1;
			}
		...
		navToPage(newpage) {
			...
			} else if (this.currentPage > this.firstVisiblePage + this.multipleBase - 1) {
				this.firstVisiblePage += this.multipleBase;
			}
			if (this.firstVisiblePage <= 0) this.firstVisiblePage = 0;

			this.showWhatToShow();
			document.getElementsByClassName("petitpois")[0].scrollIntoView({
				behavior: "smooth",
			});
			this.watch_current_page = this.currentPage;
		},
		...
		showWhatToShow() {
			...
			// 		if (this.itemContainerEl.children[i].classList.contains("pagToHide")) {
		...
		nbrOfPagesDynamic() {
			if (this.firstVisiblePage + this.multipleBase > this.nbrOfPages) {
				...
				const v = this.multipleBase >= this.nbrOfPages ? this.nbrOfPages : this.multipleBase;
